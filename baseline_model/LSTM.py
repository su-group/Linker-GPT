...
class LSTM:
    def __init__(self, config, gpu_type, pattern='train'):
        assert pattern in ['train', 'sample']
        self.tok = Tokenizer()
        self.config = config
    ...
    def init_model(self):
        self.model = LSTM(self.vocab_size, self.config.embedding_dim,
    ...
    def train(self, train_loader, valid_loader):
        ...
        for epoch in range(self.config.num_epochs):
            ...
            print(
                f"Train Loss: {train_loss_all[-1]:.4f} Train Acc: {train_acc_all[-1]:.4f}, Epoch Time(Train): {epoch_time_train:.2f}s")
            scheduler.step()
            ...
            valid_loss_all.append(avg_valid_loss)
            valid_acc_all.append(valid_corrects.float().item() / valid_num)
            valid_time = time.time()
            epoch_time_valid = valid_time - train_time
            print(
                f"Valid Loss: {avg_valid_loss:.4f} Valid Acc: {valid_acc_all[-1]:.4f}, Epoch Time (Valid): {epoch_time_valid:.2f}s")
        ...
        self.config.pretrain_model = os.path.join(self.config.prj_dir,
                                                  f'CLM-pretrain_best-{best_epoch}-{best_valid_loss:.4f}.pkl')
        save(best_model, self.config.pretrain_model)  # save the model with the best validation performance
        # print(epochs)
        # print(train_loss_all)
        # print(valid_loss_all)
        df = pd.DataFrame({'epoch':epochs,
                           'train_loss':train_loss_all,
                           'val_loss':valid_loss_all})
        df.to_csv('loss/loss_adc_linkers.csv')
