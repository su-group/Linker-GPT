...
class Trainer:
    ...
    def train(self, wandb):
        ...
        def run_epoch(split):
            ...
            for it, batch in pbar:  # ← 改为 batch，不再直接解包
                ...
                if len(batch) == 4:
                    ...
                    x, y, extra = batch
                    # 根据 config 判断 extra 是 prop 还是 scaffold
                    if getattr(config, 'num_props', 0) > 0:
                        p, scaffold = extra, None
                    else:
                        p, scaffold = None, extra
                elif len(batch) == 2:
                    x, y = batch
                    p = scaffold = None
                else:
                ...
                x = x.to(self.device)
                y = y.to(self.device)
                p = p.to(self.device) if p is not None else None
                scaffold = scaffold.to(self.device) if scaffold is not None else None
