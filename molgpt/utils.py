...
def load_model_and_vocab(checkpoint_path, device='cuda'):
    ...
    Returns:
        ...
        itos (dict): index -> token
    """
    print(f"Loading checkpoint from {checkpoint_path}")
    checkpoint = torch.load(checkpoint_path, map_location='cpu')

    # 严格检查格式
    required_keys = ['model_state_dict', 'model_config', 'stoi', 'itos']
    for key in required_keys:
        if key not in checkpoint:
            raise ValueError(f"Checkpoint missing required key: '{key}'. "
                             "Please save model using full checkpoint format.")
    ...
    config = GPTConfig(
        ...
        **{k: v for k, v in config_dict.items() 
           if k not in ['vocab_size', 'block_size']}
    )
    ...
    print("Successfully loaded model, config, and vocab from checkpoint.")
